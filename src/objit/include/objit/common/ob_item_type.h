...
extern "C"
{
  ...
  typedef enum ObItemType
  {
    ...
    T_FUN_SYS_HELLO_REPEAT = 783, // 神hello_repeat() function
...
#define IS_BOOL_OP(op) \
  (((op) >= T_OP_EQ && (op) <= T_OP_NOT_IN && (op) != T_OP_POW) || ((op) == T_OP_EXISTS) || ((op) == T_OP_NOT_EXISTS) || ((op) == T_FUN_SYS_STRCMP) || ((op) == T_FUN_SYS_JSON_VALID) || ((op) == T_FUN_SYS_JSON_CONTAINS) || ((op) == T_FUN_SYS_JSON_MEMBER_OF) || ((op) == T_FUN_SYS_JSON_OVERLAPS) || ((op) == T_FUN_SYS_JSON_CONTAINS) || ((op) == T_FUN_SYS_JSON_CONTAINS_PATH) || ((op) == T_FUN_SYS_JSON_SCHEMA_VALID) || ((op) == T_FUN_SYS_JSON_EQUAL) || ((op) == T_FUN_SYS_IS_JSON) || ((op) == T_FUN_SYS_JSON_EXISTS) || ((op) == T_BOOL) || ((op) == T_OP_XOR) || ((op) == T_FUN_SYS_IS_UUID) || ((op) == T_FUN_SYS_ST_CONTAINS) || ((op) == T_FUN_SYS_ST_WITHIN) || ((op) == T_FUN_SYS_ST_DWITHIN) || ((op) == T_FUN_SYS_ST_INTERSECTS) || ((op) == T_FUN_SYS_ST_COVERS) || ((op) == T_FUN_SYS_PRIV_ST_EQUALS) || ((op) == T_FUN_SYS_PRIV_ST_TOUCHES) || ((op) == T_FUN_SYS_ST_CROSSES) || ((op) == T_FUN_SYS_ST_OVERLAPS))

// in oracle mode, only lists exprs can accept bool(tinyint) param
#define ALLOW_BOOL_INPUT(op) \
  ((IS_BOOL_OP((op))) || ((op) == T_FUN_COLUMN_CONV) || ((op) == T_FUN_PL_COLLECTION_CONSTRUCT) || ((op) == T_FUN_PL_INTEGER_CHECKER) || ((op) == T_FUN_SYS_CAST) || ((op) == T_FUN_SYS_LNNVL) || ((op) == T_FUN_SYS_NVL) || ((op) == T_FUN_UDF) || ((op) == T_OP_ARG_CASE) || ((op) == T_OP_CASE) || ((op) == T_OP_OUTPUT_PACK) || ((op) == T_FUN_SYS_JSON_OBJECT) || ((op) == T_FUN_SYS_JSON_ARRAY) || ((op) == T_OP_TO_OUTFILE_ROW))
...
#define IS_FUN_SYS_TYPE(op) (((op) >= T_FUN_SYS && (op) < T_FUN_SYS_END) || ((op) > T_MIN_OP && (op) <= T_OP_CONST_VAL) || T_OP_CONNECT_BY_ROOT == (op) || (lib::is_oracle_mode() && (op) == T_OP_BIT_AND))

#define IS_FUN_STYLE(op) (((op) >= T_FUN_SYS && (op) < T_FUN_SYS_END) || ((op) >= T_FUN_MAX && (op) <= T_FUN_APPROX_COUNT_DISTINCT_SYNOPSIS_MERGE) || T_OP_ABS == (op) || T_OP_MOD == (op) || T_OP_POW == (op) || T_OP_CONV == (op) || T_OP_CNN == (op) || T_OP_SIGN == (op) || T_OP_CONNECT_BY_ROOT == (op))

#define ITEM_TO_OBJ_TYPE(op) (static_cast<ObObjType>(op))

#define IS_COMPARISON_OP(op) \
  (((op) >= T_OP_EQ && (op) <= T_OP_NE) || ((op) >= T_OP_SQ_EQ && (op) <= T_OP_SQ_NE) || ((op) == T_OP_IN) || ((op) == T_OP_NOT_IN))
...
#define IS_PREPARABLE_STMT(op) \
  ((op) == T_SELECT || (op) == T_DELETE || (op) == T_INSERT || (op) == T_MERGE || (op) == T_UPDATE || (op) == T_BEGIN || (op) == T_COMMIT || (op) == T_ROLLBACK)
