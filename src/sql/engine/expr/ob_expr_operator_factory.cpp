...
#include "sql/engine/expr/ob_expr_hello_repeat.h" //神
...
#define REG_SAME_OP_ORCL(OriOpType, NewOpType, NewOpName, idx_oracle)                                        \
  ...
  {                                                                                                          \
    ...
    ObExprOperatorType ObExprOperatorFactory::get_type_by_name(const ObString &name)
    {
      ...
      if (lib::is_oracle_mode())
      {
        ...
        for (uint32_t i = 0; i < ARRAYSIZEOF(NAME_TYPES_ORCL); i++)
        {
          ...
          if (static_cast<int32_t>(strlen(NAME_TYPES_ORCL[i].name_)) == func_name.length() && strncasecmp(name_buf, func_name.ptr(), func_name.length()) == 0)
          {
      ...
      else
      {
        for (uint32_t i = 0; i < ARRAYSIZEOF(NAME_TYPES); i++)
        {
          ...
          if (static_cast<int32_t>(strlen(NAME_TYPES[i].name_)) == real_func_name.length() && strncasecmp(NAME_TYPES[i].name_, real_func_name.ptr(), real_func_name.length()) == 0)
          {
    ...
    int ObExprOperatorFactory::alloc(ObExprOperatorType type, ObExprOperator *&expr_op)
    {
      ...
      else if (OB_ISNULL(lib::is_oracle_mode() ? OP_ALLOC_ORCL[type] : OP_ALLOC[type]))
      {
        ret = OB_ERR_UNEXPECTED;
        OB_LOG(WARN, "unexpectd expr item type", K(ret), K(type));
      }
      else if (OB_FAIL(lib::is_oracle_mode() ? OP_ALLOC_ORCL[type](alloc_, expr_op) : OP_ALLOC[type](alloc_, expr_op)))
      {
